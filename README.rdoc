...
As the whole pagination helper is basically just a collection of links and non-links, Pager renders each of them through its own partial template inside the Engine. So, you can easily modify their behaviour, style or whatever by overriding partial templates.

=== ORM & template engine agnostic
Pager supports multiple ORMs (ActiveRecord, Mongoid, MongoMapper) multiple web frameworks (Rails, Sinatra), and multiple template engines (ERB, Haml).
...
You can configure the following default values by overriding these values using <tt>Pager.configure</tt> method.
...
Pager includes a handy template generator.

* to edit your paginator

  Run the generator first,
    % rails g pager:views default

  then edit the partials in your app's <tt>app/views/pager/</tt> directory.
...
* multiple themes
  ...
  To utilize multiple themes from within a single application, create a directory within the app/views/pager/ and move your custom template files into that directory.
    % rails g pager:views default (skip if you have existing pager views)
    % cd app/views/pager
  ...
  Note: if the theme isn't present or none is specified, pager will default back to the views included within the gem.

=== Paginating a generic Array object

Pager provides an Array wrapper class that adapts a generic Array object to the <tt>paginate</tt> view helper.
However, the <tt>paginate</tt> helper doesn't automatically handle your Array object (this is intentional and by design).
<tt>Pager::paginate_array</tt> method converts your Array object into a paginatable Array that accepts <tt>page</tt> method.
  Pager.paginate_array(my_array_object).page(params[:page]).per(10)
...
You can specify the +total_count+ value through options Hash. This would be helpful when handling an Array-ish object that has a different +count+ value from actual +count+ such as RSolr search result or when you need to generate a custom pagination. For example:
  Pager.paginate_array([], total_count: 145).page(params[:page]).per(10)
...
Since version 0.13.0, pager started to support Sinatra or Sinatra-based frameworks experimentally.

To use pager and its helpers with these frameworks,
...
Check out Pager recipes on the GitHub Wiki for more advanced tips and techniques.
https://github.com/amatsuda/pager/wiki/Pager-recipes


== Build Status {<img src="https://travis-ci.org/amatsuda/pager.png"/>}[http://travis-ci.org/amatsuda/pager]
